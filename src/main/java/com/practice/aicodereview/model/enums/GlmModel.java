package com.practice.aicodereview.model.enums;

import lombok.Getter;

/**
 * ChatGLM 模型枚举
 * 定义所有支持的 ChatGLM 模型
 *
 * @author AI Code Review Team
 * @version 1.0.0
 */
@Getter
public enum GlmModel {

    /**
     * GLM-4-Flash - 面向 Agentic Coding 场景强化了编码能力、长程任务规划与工具协同
     */
    GLM_4_7_FLASH("GLM-4.7-Flash", "面向 Agentic Coding 场景强化了编码能力、长程任务规划与工具协同"),

    /**
     * GLM-4-Flash - 适用简单任务，速度最快，价格最实惠的版本，具有128k上下文
     */
    GLM_4_FLASH("glm-4-flash", "适用简单任务，速度最快，价格最实惠的版本，具有128k上下文"),

    /**
     * GLM-4 - 适用于复杂的对话交互和深度内容创作设计的场景
     */
    GLM_4("glm-4", "适用于复杂的对话交互和深度内容创作设计的场景"),

    /**
     * GLM-4V - 根据输入的自然语言指令和图像信息完成任务
     */
    GLM_4V("glm-4v", "根据输入的自然语言指令和图像信息完成任务，推荐使用 SSE 或同步调用方式请求接口"),

    /**
     * GLM-3-Turbo - 适用于对知识量、推理能力、创造力要求较高的场景
     */
    GLM_3_5_TURBO("glm-3-turbo", "适用于对知识量、推理能力、创造力要求较高的场景"),

    /**
     * ChatGLM-Turbo - 适用于对知识量、推理能力、创造力要求较高的场景
     */
    CHATGLM_TURBO("chatglm_turbo", "适用于对知识量、推理能力、创造力要求较高的场景"),

    /**
     * CogView-3 - 根据用户的文字描述生成图像
     */
    COGVIEW_3("cogview-3", "根据用户的文字描述生成图像,使用同步调用方式请求接口");

    private final String code;
    private final String description;

    GlmModel(String code, String description) {
        this.code = code;
        this.description = description;
    }

    /**
     * 根据 code 获取模型枚举
     *
     * @param code 模型代码
     * @return 模型枚举，未找到返回 null
     */
    public static GlmModel fromCode(String code) {
        for (GlmModel model : values()) {
            if (model.getCode().equals(code)) {
                return model;
            }
        }
        return null;
    }
}
